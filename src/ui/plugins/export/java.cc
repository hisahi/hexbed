...
/*                                                                          */
/****************************************************************************/
// ui/plugins/export/java.cc -- impl for builtin Java source code exporter
...
static constexpr unsigned JAVA_MAX_COLUMNS = 1000;
static constexpr unsigned JAVA_MAX_BYTES_PER_LINE = 256;
...
class ExportPluginJavaDialog : public PluginConfigureDialog<true> {
  ...
  public:
    ExportPluginJavaDialog(wxWindow* parent, ExportPluginJavaSettings& settings)
        ...
        okButton->Bind(wxEVT_BUTTON, &ExportPluginJavaDialog::OnOK, this);
    ...
    bool CheckValid() noexcept override {
        unsigned byteWidth = settings_->useHex ? 7 : 5;
...
ExportPluginJava::ExportPluginJava(pluginid id)
    : LocalizableExportPlugin(id, TAG("Java source code")) {}

wxString ExportPluginJava::getFileFilter() const {
    return _("Java source code file (*.java)") + "|*.java|" +
           ExportPlugin::getFileFilter();
}

bool ExportPluginJava::configureExport(wxWindow* parent,
...
static unsigned baseIndent(std::ofstream& f,
                           const ExportPluginJavaSettings& settings) {
    unsigned n = 0;
...
static void doExportRigid_(HexBedTask& task, std::ofstream& f,
                           ...
                           const ExportPluginJavaSettings& settings) {
    bufsize o = 0;
    byte row[JAVA_MAX_BYTES_PER_LINE];
...
void ExportPluginJava::doExport(HexBedTask& task,
                                const std::filesystem::path& filename,
                                std::function<bufsize(bufsize, bytespan)> read,
                                bufsize actualOffset, bufsize size,
                                const ExportDetails& details) {
